...
```{r}
test_x = as.matrix(cbind(dummy_cols(test[,c(2:3,6:19,22:26,28:34,36,40:43,54,56,58,59,61,64:66,73:75,77:80)], remove_selected_columns = T, remove_first_dummy = T), test[,-c(2:3,6:19,22:26,28:34,36,40:43,54,56,58,59,61,64:66,73:75,77:81)]))

test$lasso_pred = exp(predict(lasso, test_x))

test$mars_pred = exp(predict(mars, test[,-81]))
```

```{r}
sum((test$lasso_pred - test$SalePrice)^2)
sum((test$mars_pred - test$SalePrice)^2)
```
